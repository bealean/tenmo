...
--/
CREATE OR REPLACE PROCEDURE create_transfer(IN "senderBalance" numeric, IN "senderAccount" bigint, IN "recipientBalance" numeric, IN "recipientAccount" bigint, IN "transferStatusDesc" character varying, IN "transferTypeDesc" character varying, IN amount numeric, INOUT transfer_id integer)
LANGUAGE 'sql'
    
AS $BODY$
UPDATE accounts SET balance = "senderBalance" WHERE account_id = "senderAccount";
UPDATE accounts SET balance = "recipientBalance" WHERE account_id = "recipientAccount";
INSERT into transfers (transfer_type_id, transfer_status_id, account_from, account_to, amount) 
                VALUES ((SELECT transfer_type_id FROM transfer_types WHERE transfer_type_desc = "transferTypeDesc"),
                (SELECT transfer_status_id FROM transfer_statuses 
				 WHERE transfer_status_desc = "transferStatusDesc"), "senderAccount", "recipientAccount", "amount") 
